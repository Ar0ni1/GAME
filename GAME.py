...
def sborkamecha(player_name, teacher_name):
    print(f"{teacher_name}: {player_name} теперь из кайбер-кристалла тебе необходимо сделать световой меч\n"
          ...
          f"{teacher_name}: В этом тебе поможет выскотехнологичный робот Хуян")
    print(
        f"Хуян: Здравствуй, юный падаван! Я - робот Хуян. За все время на своем корабле я создал более 10,000 световых мечей для падаванов. Давай соберем для тебя блестящий световой меч.")
    tipsvetovogomecha = input(f"Хуян: Начнем с простого. Какого типа меч ты хочешь (Обычный / Двусторонний / 2 меча): ")
    cvetlezviya = input(f"Хуян: Отличный выбор! Теперь выбери цвет лезвия (Синий / Зелёный / Фиолетовый): ")
    materialrukoitki = input(f"Хуян: Хорошо, теперь выбери материал для рукоятки (Металл / Дерево / Пластик): ")
    print(f"Хуян: Великолепно, {player_name}! Твой световой меч готов.")
